############################################
# PROJECT 4: Developing Customized Product Recommendations
############################################

# Güncellendi:
# create_cltv_p fonksiyonu güncellendi.

# Produce Association rules using location and segments and make recommendations using these rules.

# Use Germany as Country? Germany (2010-2011)
...
#########################################################
# Task 1: Downloading the data from distant server
#########################################################
...
######################################
# Task 2: Preprocess the data using crm_data_prep function
######################################
df.head()
...
######################################
# Task 3: Form Predictive CLTV Segments using create_cltv_p function
######################################

def create_cltv_p(dataframe):
    today_date = dt.datetime(2011, 12, 11)

    ## recency dynamic to customer.
    ...
    # Calculation of WEEKLY RECENCY & WEEKLY T for BGNBD
    ...
    ## recency filter (for accurate cltvp calculation )
    ...
    # exp_sales_1_month
    rfm["exp_sales_1_month"] = bgf.predict(4*1,
                                           rfm['frequency'],
                                           rfm['recency_weekly_cltv_p'],
                                           rfm['T_weekly'])
    # exp_sales_3_month
    rfm["exp_sales_3_month"] = bgf.predict(4*3,
                                           rfm['frequency'],
                                           rfm['recency_weekly_cltv_p'],
                                           rfm['T_weekly'])
...
######################################
# Task 4: Reduce the data set using ID's of customers in requested segment.
######################################
# Obtaining ID's
...
# Reducing df according to IDs bu id'lere göre df'lerin indirgenmesi
...
######################################
# Task-5:Produce Association rules of each segment
######################################
...
######################################
# Task 6: Make recommendations for German customers according to segments
######################################
# add a column named as recommended_product into cltv_p  dataframe.
# add a product for each segment.
# Add a rule described above to the customer regarding their segments.
